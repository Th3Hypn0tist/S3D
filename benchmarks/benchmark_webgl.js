...
class WebGLBenchmark {
  constructor(canvas, output) {
    ...
    if (!this.gl) throw new Error('WebGL2 required for benchmark');
    this.metrics = new FrameMetrics(300);
  ...
  pan(dx, dy) {
    ...
    const forward = Vec3.norm([-Math.sin(yaw) * cp, -Math.sin(this.pitch), -Math.cos(yaw) * cp]);
    let right = Vec3.cross(forward, [0, 1, 0]);
    right = Vec3.length(right) < 1e-6 ? [1, 0, 0] : Vec3.norm(right);
    const up = Vec3.norm(Vec3.cross(right, forward));
    const scale = this.distance * .0015;
    this.target = Vec3.add(this.target, Vec3.add(Vec3.mul(right, -dx * scale), Vec3.mul(up, dy * scale)));
  }
  zoom(deltaY) { this.distance = Math.max(3, Math.min(320, this.distance * Math.exp(deltaY * .001))); }
  ...
  loadPreset(preset) {
    ...
    const positions = Benchmark.nodePositions(preset.nodes);
    const links = Benchmark.linkVertices(positions, preset.links);
    this.frameUploads = 0;
  ...
  viewProjection() {
    ...
    return Mat4.multiply(Mat4.perspective(55, aspect, .1, 500), Mat4.lookAt(this.cameraPosition(), this.target));
  }
